 ...
 * - from                               User                          Optional  Sender of the message; empty for messages sent to channels. For backward compatibility, the field contains a fake sender user in non-channel chats,
 *                                                                                  if the message was sent on behalf of a chat.
 * - sender_chat                        Chat                          Optional  Sender of the message, sent on behalf of a chat. For example, the channel itself for channel posts, the supergroup itself for messages from 
 *                                                                                  anonymous group administrators, the linked channel for messages automatically forwarded to the discussion group. For backward compatibility, 
 ...
 * - sender_business_bot                User                          Optional  The bot that actually sent the message on behalf of the business account. Available only for outgoing messages sent on behalf of the 
 ...
 * - business_connection_id             String                        Optional  Unique identifier of the business connection from which the message was received. If non-empty, the message belongs to a chat of the 
 *                                                                                  corresponding business account that is independent from any potential bot chat which might share the same identifier.
 ...
 * - reply_to_message                   Message                       Optional  For replies in the same chat and message thread, the original message. Note that the Message object in this field will not contain 
 ...
 * - supergroup_chat_created            True                          Optional  Service message: the supergroup has been created. This field can't be received in a message coming through updates, because bot can't be 
 *                                                                                  a member of a supergroup when it is created. It can only be found in reply_to_message if someone replies to a very first message in
 *                                                                                  a directly created supergroup.
 * - channel_chat_created               True                          Optional  Service message: the channel has been created. This field can't be received in a message coming through updates, because bot can't be
 *                                                                                  a member of a channel when it is created. It can only be found in reply_to_message if someone replies to a very first message in a channel.
 ...
 * - migrate_to_chat_id                 Integer                       Optional  The group has been migrated to a supergroup with the specified identifier. This number may have more than 32 significant bits and 
 *                                                                                  some programming languages may have difficulty/silent defects in interpreting it. But it has at most 52 significant bits, 
 *                                                                                  so a signed 64-bit integer or double-precision float type are safe for storing this identifier.
 * - migrate_from_chat_id               Integer                       Optional  The supergroup has been migrated from a group with the specified identifier. This number may have more than 32 significant bits and 
 *                                                                                  some programming languages may have difficulty/silent defects in interpreting it. But it has at most 52 significant bits, 
 *                                                                                  so a signed 64-bit integer or double-precision float type are safe for storing this identifier.
 ...
 * - write_access_allowed               WriteAccessAllowed            Optional  Service message: the user allowed the bot to write messages after adding it to the attachment or side menu, launching a Web App from a link, 
...
class Message {
  public  int                  $Id;          // Integer                           Unique message identifier inside this chat
  public ?User                 $From;        // User                    Optional  Sender of the message; empty for messages sent to channels. For backward compatibility, the field contains a fake sender user in non-channel chats, if the message was sent on behalf of a chat.
  public  Chat                 $Chat;        // Chat                              Chat the message belongs to
  public  string               $Date;        // Integer                           Date the message was sent in Unix time. It is always a positive number, representing a valid date.
  public ?string               $Text;        // String                  Optional  For text messages, the actual UTF-8 text of the message
  public ?array                $Entities;    // Array of MessageEntity  Optional  For text messages, special entities like usernames, URLs, bot commands, etc. that appear in the text
  public ?InlineKeyboardMarkup $ReplyMarkup; // InlineKeyboardMarkup	  Optional  Inline keyboard attached to the message. login_url buttons are represented as ordinary url buttons.
  ...
  function __construct(array $Message){
    ...
    $this->Entities    = isset( $Message['entities'] ) ? $Message['entities'] : null;
    $this->ReplyMarkup = isset( $Message['reply_markup'] ) ? new InlineKeyboardMarkup( $Message['reply_markup'] ) : null;
  }
